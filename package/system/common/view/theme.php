...
$arrayInfo = $translator->getFileInfo(basename($_SERVER['PHP_SELF']));
                    ...
                    if (is_array($themeArray)) {
                        ...
                        if ($totalRecord > 0) {
                            for ($i = 0; $i < $totalRecord; $i++) {
                                echo "<tr>";
                                echo "<td>
		<a class='btn-warning btn-mini' onClick=showFormUpdate('" . $theme->getViewPath();
                                ?>','<?php echo $securityToken; ?>','<?php
                    echo intval($themeArray [$i]['themeId']) . "')><i class='icon-edit icon-white'></i>Update</a>  
