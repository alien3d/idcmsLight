...
/**
 * this is skill setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author Maq,hafizan
 ...
 */
class SkillClass extends \Core\ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Common\Skill\Model\SkillModel();
	...
	public function create() {
		...
		$end = microtime(true);
		$time = $end - $start;
		echo json_encode(array("success" => true, "message" =>  $this->systemString->getCreateMessage(), "skillId" => $skillId));
		exit();
	}
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	`skill`.`isActive`		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			if ($this->model->getSkillId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getSkillId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT		[skill].[skillId],
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getSkillId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getSkillId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('skillId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('skill');
				...
				$sql = "
							...
							(
								SELECT 		[skill].[skillId],
											...
											[skill].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [skillId]) AS 'RowNumber'
								FROM 	[skill]
								JOIN		[staff]
								ON		[skill].[executeBy] = [staff].[staffId]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getSkillId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`skill`
				SET 		`skillCode`		=	'" . $this->model->getSkillCode() . "',
							`skillDesc`			=	'" . $this->model->getSkillDesc() . "',
			...
			$this->q->tableName = $this->model->getTableName();
			$this->q->primaryKeyName = $this->model->getPrimaryKeyName();
			$this->q->primaryKeyValue = $this->model->getSkillId(0, 'single');
			$this->q->audit = $this->audit;
			$this->q->update($sql);
			if ($this->q->execute == 'fail') {
				echo json_encode(array("success" => false, "message" => $this->q->responce));
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getSkillId($i, 'array') . "'
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['skillDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "skill" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['databaseRequest'])) {
		$skillObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$skillObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$skillObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$skillObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$skillObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$skillObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$skillObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$skillObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$skillObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$skillObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$skillObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	 ...
	 */
	if (isset($_GET ['skillDesc'])) {
		if (strlen($_GET ['skillDesc']) > 0) {
