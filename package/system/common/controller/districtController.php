...
/**
 * this is district setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author Maq,Hafizan
 ...
 */
class DistrictClass extends \Core\ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Common\District\Model\DistrictModel();
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			VALUES
					(
						'" . $this->model->getDistrictCode() . "',				'".$this->model->getDistrictDesc()."',	
	...
	public function read() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			if ($this->model->getDistrictId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getDistrictId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT		[district].[districtId],
			...
			SELECT		DISTRICT.DISTRICTID   		 	AS 	\"districtId\",
						...
						DISTRICT.DISTRICTDESC			AS 	\"districtDesc\",
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getDistrictId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getDistrictId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('districtId','stateId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('district','state');
				...
				$sql = "
							...
							(
								SELECT 		[district].[districtId],
											...
											[district].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [districtId]) AS 'RowNumber'
								FROM 	[district]
								JOIN		[staff]
								ON		[district].[executeBy] = [staff].[staffId]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getDistrictId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[" . $this->model->getPrimaryKeyName() . "]
			FROM 	[" . $this->model->getTableName() . "]
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDistrictId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`district`
				SET 		`districtCode`		=	'" . $this->model->getDistrictCode() . "',
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getDistrictId($i, 'array') . "'
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['districtDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "district" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$districtObject->setIsAdmin($_POST ['isAdmin']);
	}
	/**
	 * Database Request
	 */
	if (isset($_POST ['databaseRequest'])) {
		$districtObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$districtObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$districtObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$districtObject->setFieldQuery($_POST ['query']);
	}
	...
	if (isset($_POST ['order'])) {
		$districtObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$districtObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$districtObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['districtDesc'])) {
		if (strlen($_GET ['districtDesc']) > 0) {
