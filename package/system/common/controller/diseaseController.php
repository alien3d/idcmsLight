...
/**
 * this is disease setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author Maq,hafizan
 ...
 */
class DiseaseClass extends \Core\ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Common\Disease\Model\DiseaseModel();
	...
	public function create() {
		...
		$end = microtime(true);
		$time = $end - $start;
		echo json_encode(array("success" => true, "message" =>  $this->systemString->getCreateMessage(), "diseaseId" => $diseaseId));
		exit();
	}
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	`disease`.`isActive`		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			if ($this->model->getDiseaseId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getDiseaseId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT		[disease].[diseaseId],
			...
			SELECT		DISEASE.DISEASEID   		 	AS 	\"diseaseId\",
						...
						DISEASE.DISEASEDESC	AS 	\"diseaseDesc\",							
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getDiseaseId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getDiseaseId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('diseaseId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('disease');
				...
				$sql = "
							...
							(
								SELECT 		[disease].[diseaseId],
											...
											[disease].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [diseaseId]) AS 'RowNumber'
								FROM 	[disease]
								JOIN		[staff]
								ON		[disease].[executeBy] = [staff].[staffId]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDiseaseId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`disease`
				SET 		`diseaseCode`		=	'" . $this->model->getDiseaseCode() . "',
							`diseaseDesc`			=	'" . $this->model->getDiseaseDesc() . "',
			...
			$this->q->tableName = $this->model->getTableName();
			$this->q->primaryKeyName = $this->model->getPrimaryKeyName();
			$this->q->primaryKeyValue = $this->model->getDiseaseId(0, 'single');
			$this->q->audit = $this->audit;
			$this->q->update($sql);
			if ($this->q->execute == 'fail') {
				echo json_encode(array("success" => false, "message" => $this->q->responce));
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getDiseaseId($i, 'array') . "'
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['diseaseDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "disease" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$diseaseObject->setIsAdmin($_POST ['isAdmin']);
	}
	/**
	 * Database Request
	 */
	if (isset($_POST ['databaseRequest'])) {
		$diseaseObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$diseaseObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$diseaseObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$diseaseObject->setFieldQuery($_POST ['query']);
	}
	...
	if (isset($_POST ['order'])) {
		$diseaseObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$diseaseObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$diseaseObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['diseaseDesc'])) {
		if (strlen($_GET ['diseaseDesc']) > 0) {
