...
/**
 * this is family setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author Maq,hafizan
 ...
 */
class FamilyClass extends \Core\ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Common\Family\Model\FamilyModel();
	...
	public function create() {
		...
		$time = $end - $start;
		$this->q->commit();
		echo json_encode(array("success" => true, "message" =>  $this->systemString->getCreateMessage(), "familyId" => $familyId));
		exit();
	}
	...
	public function read() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			if ($this->model->getFamilyId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getFamilyId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[family].[familyId],
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getFamilyId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getFamilyId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('familyId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('family');
				...
				$sql = "
							...
							(
								SELECT 		[family].[familyId],
											...
											[family].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [familyId]) AS 'RowNumber'
								FROM 	[family]
								JOIN		[staff]
								ON		[family].[executeBy] = [staff].[staffId]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getFamilyId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`family`
				SET 		`familyCode`		=	'" . $this->model->getFamilyCode() . "',				
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getFamilyId($i, 'array') . "'
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['familyDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "family" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$familyObject->setIsAdmin($_POST ['isAdmin']);
	}
	/**
	 * Database Request
	 */
	if (isset($_POST ['databaseRequest'])) {
		$familyObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$familyObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$familyObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$familyObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$familyObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$familyObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$familyObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$familyObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$familyObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$familyObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$familyObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$familyObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['familyDesc'])) {
		if (strlen($_GET ['familyDesc']) > 0) {
