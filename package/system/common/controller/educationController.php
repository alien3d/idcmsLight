...
/**
 * this is education setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author Maq,hafizan
 ...
 */
class EducationClass extends \Core\ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Common\Education\Model\EducationModel();
	...
	public function read() {
		...
		if ($this->getIsAdmin() == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	`education`.`isActive`		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			if ($this->model->getEducationId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getEducationId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[education].[educationId],
			...
			SELECT		EDUCATION.EDUCATIONID   		 	AS 	\"educationId\",
						...
						EDUCATION.EDUCATIONDESC			AS \"educationDesc\",
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getEducationId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getEducationId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('educationId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('education');
				...
				$sql = "
							...
							(
								SELECT 		[education].[educationId],
											...
											[education].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [educationId]) AS 'RowNumber'
								FROM 	[education]
								JOIN		[staff]
								ON		[education].[executeBy] = [staff].[staffId]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getEducationId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[" . $this->model->getPrimaryKeyName() . "]
			FROM 	[" . $this->model->getTableName() . "]
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getEducationId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`education`
				SET 		`educationCode`		=	'" . $this->model->getEducationCode() . "',				
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getEducationId($i, 'array') . "'
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['educationDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "education" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$educationObject->setIsAdmin($_POST ['isAdmin']);
	}
	/**
	 * Database Request
	 */
	if (isset($_POST ['databaseRequest'])) {
		$educationObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$educationObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$educationObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$educationObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$educationObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$educationObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$educationObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$educationObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$educationObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$educationObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$educationObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	 ...
	 */
	if (isset($_GET ['educationDesc'])) {
		if (strlen($_GET ['educationDesc']) > 0) {
