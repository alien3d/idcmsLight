...
class RaceClass extends \Core\ConfigClass {
	...
	public function create() {
		...
		$end = microtime(true);
		$time = $end - $start;
		echo json_encode(array("success" => true, "message" =>  $this->systemString->getCreateMessage(), "raceId" => $raceId));
		exit();
	}
				...
				$sql = "
							...
							(
								SELECT 		[race].[raceId],
											...
											[race].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [raceId]) AS 'RowNumber'
								FROM 	[race]
								JOIN		[staff]
								ON		[race].[executeBy] = [staff].[staffId]
	...
	function update() {
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`race`
				SET 		`raceCode`		=	'" . $this->model->getRaceCode() . "',
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['raceDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$raceObject->setIsAdmin($_POST ['isAdmin']);
	}
	/**
	 * Database Request
	 */
	if (isset($_POST ['databaseRequest'])) {
		$raceObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$raceObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$raceObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$raceObject->setFieldQuery($_POST ['query']);
	}
	...
	if (isset($_POST ['dateRangeStart'])) {
		$raceObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$raceObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$raceObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$raceObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$raceObject->setSortField($_POST ['sortField']);
	}
	/*
