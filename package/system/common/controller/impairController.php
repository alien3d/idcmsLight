...
require_once ("../model/impairModel.php");

/**
 * this is impair setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author Maq,hafizan
 ...
 */
class ImpairClass extends \Core\ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Common\Impair\Model\ImpairModel();
	...
	public function create() {
		...
		$end = microtime(true);
		$time = $end - $start;
		echo json_encode(array("success" => true, "message" =>  $this->systemString->getCreateMessage(), "impairId" => $impairId));
		exit();
	}
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	`impair`.`isActive`		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT		IMPAIR.IMPAIRID   		 	AS 	\"impairId\",
						...
						IMPAIR.IMPAIRDESC	AS 	\"impairDesc\",							
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getImpairId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getImpairId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('impairId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('impair');
				...
				$sql = "
							...
							(
								SELECT 		[impair].[impairId],
											...
											[impair].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [impairId]) AS 'RowNumber'
								FROM 	[impair]
								JOIN		[staff]
								ON		[impair].[executeBy] = [staff].[staffId]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getImpairId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`impair`
				SET 		`impairCode`		=	'" . $this->model->getImpairCode() . "',
							`impairDesc`			=	'" . $this->model->getImpairDesc() . "',
			...
			$this->q->tableName = $this->model->getTableName();
			$this->q->primaryKeyName = $this->model->getPrimaryKeyName();
			$this->q->primaryKeyValue = $this->model->getImpairId(0, 'single');
			$this->q->audit = $this->audit;
			$this->q->update($sql);
			if ($this->q->execute == 'fail') {
				echo json_encode(array("success" => false, "message" => $this->q->responce));
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getImpairId($i, 'array') . "'
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['impairDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "impair" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$impairObject->setIsAdmin($_POST ['isAdmin']);
	}
	/**
	 * Database Request
	 */
	if (isset($_POST ['databaseRequest'])) {
		$impairObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$impairObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$impairObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$impairObject->setFieldQuery($_POST ['query']);
	}
	...
	if (isset($_POST ['dateRangeStart'])) {
		$impairObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$impairObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$impairObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$impairObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$impairObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$impairObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['impairDesc'])) {
		if (strlen($_GET ['impairDesc']) > 0) {
