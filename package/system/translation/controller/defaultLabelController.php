<?php
namespace Core\System\Translation\DefaultLabel\Controller;
session_start ();
...
class DefaultLabelClass extends ConfigClass {
	...
	function read() {
		...
		if ($this->getVendor () == self::MYSQL) {
			...
			WHERE 1";
			if ($this->model->getDefaultLabelId ( 0, 'single' )) {
				$sql .= " AND " . strtoupper ( $this->model->getTableName () ) . "`." . strtoupper ( $this->model->getPrimaryKeyName () ) . "='" . $this->model->getDefaultLabelId ( 0, 'single' ) . "'";
			}
		}
		/**
		...
		if ($this->model->getDefaultLabelId ( 0, 'single' )) {
			...
			array (	'success' => true,
					...
					'time' => $time, 
            			...
            			'previousRecord' => $this->recordSet->previousRecord('value', $this->model->getDefaultLabelId(0, 'single')), 
            			'nextRecord' => $this->recordSet->nextRecord('value', $this->model->getDefaultLabelId(0, 'single')), 
					...
					for ($i = 0; $i < $loop; $i++) {
						...
						if(!$this->getIsAdmin()){
								foreach ($accessClear as $clear){
									...
									$sqlLooping .= "
							WHEN '" . $this->model->$this->model->getDefaultLabelId($i, 'array') . "'
							THEN '0'";
									$sqlLooping .= " END,";
								}
...
if (isset ( $_POST ['method'] )) {
	...
	if (isset($_POST ['databaseRequest'])) {
		$defaultLabelObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$defaultLabelObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$defaultLabelObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
...
if (isset ( $_GET ['method'] )) {
	...
	if (isset($_GET ['databaseRequest'])) {
		$languageObject->setRequestDatabase($_GET ['databaseRequest']);
	}
	/*
	 * Admin Only
	 */
