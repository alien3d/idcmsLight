<?php
namespace Core\System\Translation\TableMapping\Translate\Controller;
session_start ();
...
class TableMappingTranslateClass extends ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Translation\TableMapping\Translate\Model\TableMappingTranslateModel();
						...
						if (strlen($this->model->getIsDelete($i, 'array')) > 0) {
							...
							if(!$this->getIsAdmin()){
								foreach ($accessClear as $clear){
									...
									$sqlLooping .= "
							WHEN '" . $this->model->getTableMappingTranslateId($i, 'array') . "'
							THEN '0'";
									$sqlLooping .= " END,";
								}
...
if (isset ( $_GET ['method'] )) {
	...
	if (isset($_POST ['databaseRequest'])) {
		$tableMappingTranslateObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Load the dynamic value
	 */
