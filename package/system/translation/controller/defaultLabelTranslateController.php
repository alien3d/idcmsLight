<?php
namespace Core\System\Translation\DefaultLabel\Translate\Controller;
session_start ();
...
class DefaultLabelTranslationClass extends ConfigClass {
	...
	function execute() {
		...
		$this->model = new \Core\System\Translation\DefaultLabel\Translate\Model\DefaultLabelTranslateModel();
					...
					for ($i = 0; $i < $loop; $i++) {
						...
						if(!$this->getIsAdmin()){
								foreach ($accessClear as $clear){
									...
									$sqlLooping .= "
							WHEN '" . $this->model->$this->model->getDefaultLabelTranslateId($i, 'array') . "'
							THEN '0'";
									$sqlLooping .= " END,";
								}
...
if (isset ( $_POST ['method'] )) {
	...
	if (isset($_POST ['databaseRequest'])) {
		$defaultLabelTranslationObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
