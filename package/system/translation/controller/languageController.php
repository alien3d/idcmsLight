<?php
namespace Core\System\Translation\Language\Controller;
session_start ();
					...
					for ($i = 0; $i < $loop; $i++) {
						...
						if(!$this->getIsAdmin()){
								foreach ($accessClear as $clear){
									...
									$sqlLooping .= "
							WHEN '" . $this->model->$this->model->getLanguageId($i, 'array') . "'
							THEN '0'";
									$sqlLooping .= " END,";
								}
...
if (isset ( $_POST ['method'] )) {
	...
	if (isset($_POST ['databaseRequest'])) {
		$languageObject->setRequestDatabase($_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
