...
class Vendor
{
	...
	public function connect($connection, $username, $database, $password)
	{
		...
		$this->link         = db2_connect($this->databaseName, $this->username, $this->password, $this->databaseName);
		if (!$this->link) {
			$this->execute = 'fail';
			if (db2_conn_error()) {
				$this->responce = db2_conn_error();
	...
	{
		db2_autocommit($this->link, FALSE);
	}
	/**
	 * query database
	 * @param string $sql
	...
	private function query($sql)
	{
		...
		$this->sql         = $sql;
		$error             = 0;
		$this->result      = db2_exec($this->link, $this->sql);
		if (!$this->result) {
			$this->execute 	=	'fail';
			$this->responce = 	"Sql Stament Error" . $this->sql . " \n\r" . db2_conn_error($this->link) . " <br> Error Code :x " . db2_conn_error ($this->link);
			$error          = 	1;
		}
		if ($error == 1) {
			...
			if (!$result_row) {
				$this->responce=$sql_log."<br>".db2_conn_error($this->link)."<br> Error Code :y ".db2_conn_error ($this->link);
			}
		}
	...
	private function module($operation)
	{
		...
		if (!$result) {
			...
			$result_row = db2_num_rows($result);
		}
		if ($result_row == 1) {
			$access = 'Granted';
		} elseif ($result_row == 0) {
		...
		if ($result_row == 0  || $this->log == 1) {
			...
			if (!$test1) {
				$this->execute     = 'fail';
				$this->responce = $sql_log . "[" . db2_conn_error($this->link) . "]";
			}
		}
		return ($result_row);
	}
		...
		if (strlen($sql) > 0) {
			if ($this->module('leafAccessCreateValue') == 1) {
				...
				if($this->audit==1){
					...
					$resultColumn    = db2_exec($this->link, $sqlColumn);
					if (!$resultColumn) {
						$this->execute     = 'fail';
		...
		if (strlen($sql) > 0) {
			if ($this->module('leafAccessUpdateValue') == 1) {
				if ($this->audit == 1) {
					...
					if (!$resultPrevious) {
						$this->execute     = 'fail';
						$this->responce = db2_conn_errormsg ($this->link) . "Error Code" . db2_conn_error ($this->link);
					} else {
						// successfully
	...
	public function file($sql)
	{
		 ...
		 */
		if (strlen($sql) > 0) {
			$result = db2_exec($this->link, $this->sql);
			return $result;
		} else {
			$this->execute     = 'fail';
			$this->responce = "Where's the query forgot Ya!";
	...
	{
		if ($result) {
			if(db2_num_rows($result)) {
				$this->countRecord = db2_num_rows($result);
			} else {
				$this->countRecord =0;
			}
		} else {
			if(db2_num_rows($this->result)){
				$this->countRecord = db2_num_rows($this->result);
			} else {
				$this->countRecord=0;
			}
		}
	...
	{
		//return db2_affected_rows($this->link);
	...
	{
		db2_rollback($this->link);
		$this->execute = 'fail';
	}
	/**
	 * Returns an associative array that corresponds to the fetched row and moves the internal data pointer ahead. db2_fetch_assoc() is equivalent to calling db2_fetch_array() with db2_ASSOC for the optional second parameter. It only returns an associative array
	 * @return array
	...
	{
		if ($this->result) {
			return @db2_fetch_array($this->result);
		}
		if ($result) {
	...
	{
		return db2_escape_string($this->link, $data);
	}
	/**
