...
require_once (\"../model/".$targetTable."Model.php\");

/**
 * this is ".$targetTable." setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package sample
 * @subpackage ".$targetTable."
...
class ".ucfirst($targetTable)."Class extends ConfigClass {
	...
	public function create() {
		...
		\$this->model->create();
		if (\$this->getVendor() == self::MYSQL) {";
		$controller.=$mysqlInsertStatement;	
		$controller .= "} else if (\$this->getVendor() == self::MSSQL) {";
		$controller.=$mssqlInsertStatement;
		$controller .= "} else if (\$this->getVendor() == self::ORACLE) {";
		$controller.=$oracleInsertStatement;	
		
		$controller .= "} else if (\$this->getVendor() == self::DB2) {";
		$controller.=$oracleInsertStatement;	
		
		$controller .= "} else if (\$this->getVendor() == self::POSTGRESS) {";
		$controller.=$oracleInsertStatement;	
		$controller .= "
		}";
	...
	public function read() {
		...
		if (\$this->getIsAdmin() == 0) {
			if (\$this->q->vendor == self::MYSQL) {
				\$this->auditFilter = \"	`".$targetTable."`.`isActive`		=	1	\";
		...
		$controller.=$mssqlReadStatement;
		$controller .= "if (\$this->model->get".ucfirst($targetTableId)."(0, 'single')) {
				\$sql .= \" AND [\" . \$this->model->getTableName() . \"].[\" . \$this->model->getPrimaryKeyName() . \"]		=	'\" . \$this->model->get".ucfirst($targetTableId)."(0, 'single') . \"'\";
			}
			
		} else if (\$this->getVendor() == self::ORACLE) {";
		$controller.=$oracleReadStatement;
		$controller .= "if (\$this->model->get".ucfirst($targetTableId)."(0, 'single')) {
		 ...
		 */
		\$filterArray = null;
		\$filterArray = array('".$targetTableId."');
		/**
		 * filter table
		 * @variables \$tableArray
		 */
		\$tableArray = null;
		\$tableArray = array('".$targetTable."');
		...
		if (\$this->getStart() && \$this->getLimit()) {
			...
			if (\$this->getVendor() == self::MYSQL) {
				 ...
				 */";
			$controller .=$mssqlReadPagingStatement;
			$controller .= "} else if (\$this->getVendor() == self::ORACLE) {
				/**
				 * Oracle using derived table also
				 */";
			$controller .=$oracleReadPagingStatement;	
			$controller .= "} else {
				echo json_encode(array(\"success\" => false, \"message\" => \$this->systemString->getNonSupportedDatabase()));
				exit();
			}
...
$controller .= "
	...
	function update() {
		...
		FROM 	`".$targetDbObject."`.`\" . \$this->model->getTableName() . \"`
		...
		if (\$total == 0) {
			...
			if (\$this->getVendor() == self::MYSQL) {";
			$controller.=$mysqlUpdateStatement;
			$controller .= "} else if (\$this->getVendor() == self::MSSQL) {";
			$controller.=$mssqlUpdateStatement;
			$controller .= "} else if (\$this->getVendor() == self::ORACLE) {";
			$controller.=$oracleUpdateStatement;
			$controller .= "} else if (\$this->getVendor() == self::DB2) {";
			$controller.=$oracleUpdateStatement;
			$controller.="} else if (\$this->getVendor() == self::POSTGRESS) {";
			$controller.=$oracleUpdateStatement;
	...
	function delete() {
		...
		if (\$total == 0) {
			...
			if (\$this->getVendor() == self::MYSQL) {";
			$controller.=$mysqlDeleteStatement;
			$controller .= "} else if (\$this->getVendor() == self::MSSQL) {";
			$controller.=$mssqlDeleteStatement;
			$controller .= "} else if (\$this->getVendor() == self::ORACLE) {";
			$controller.=$oracleDeleteStatement;
			$controller .= "} else if (\$this->getVendor() == self::DB2) {";
			$controller.=$oracleDeleteStatement;
			$controller .= "} else if (\$this->getVendor() == self::POSTGRESS) {";
			$controller.=$oracleDeleteStatement;
	...
	function updateStatus() {
		...
		if (\$this->getVendor() == self::MYSQL) {
			\$sql = \"
			UPDATE `".$targetDbObject."`.`\" . \$this->model->getTableName() . \"`
			SET\";
		} else if (\$this->getVendor() == self::MSSQL) {
			\$sql = \"
			UPDATE 	[".$targetDbObject."].[\" . \$this->model->getTableName() . \"]
	...
	function duplicate() {
		...
		if (\$this->getVendor() == self::MYSQL) {
			...
			\$sql = \"
			SELECT	".strtoupper('referenceNo')."
			FROM 	".strtoupper($targetTable)."
			WHERE 	".strtoupper('referenceNo')."	= 	'\" . \$this->model->getReferenceNo() . \"'
			AND		ISACTIVE			=	1\";
		}
		\$this->q->read(\$sql);
		\$total = 0;
	...
	function excel() {
		...
		\$filename = \"".$targetTable."\" . rand(0, 10000000) . \".xlsx\";
...
\$".$targetTable."Object = new ".ucfirst($targetTable)."Class ();
/**
...
if (isset(\$_POST ['method'])) {
	...
	if (isset(\$_POST ['databaseRequest'])) {
		\$".$targetTable."Object->setRequestDatabase(\$_POST ['databaseRequest']);
	}
	/*
	 *  Paging
	 */
	...
	if (isset(\$_POST ['dateRangeStart'])) {
		...
		\$".$targetTable."Object->setStartDay(\$start[2]);
		\$".$targetTable."Object->setStartMonth(\$start[1]);
		\$".$targetTable."Object->setStartYear(\$start[0]);
	}
	if (isset(\$_POST ['dateRangeEnd'])) {
		\$".$targetTable."Object->setDateRangeEndQuery(\$_POST['dateRangeEnd']);
		//explode the data to get day,month,year
		\$start=explode(\"-\",\$_POST ['dateRangeEnd']);
		\$".$targetTable."Object->setEndDay(\$start[2]);
		\$".$targetTable."Object->setEndMonth(\$start[1]);
		\$".$targetTable."Object->setEndYear(\$start[0]);
	}
...
if (isset(\$_GET ['method'])) {
	...
	if (isset(\$_GET ['databaseRequest'])) {
		\$".$targetTable."Object->setRequestDatabase(\$_GET ['databaseRequest']);
	}
	/*
	 *  Load the dynamic value
	 */
